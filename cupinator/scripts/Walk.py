...
class Walker:
    ...
    def __init__(self, accept_command=True):
        ...
        self.walkPub = rospy.Publisher('/cupinator/walk/result', WalkResult, queue_size=10)
    ...
    def checkDistanceTraveled(self, odom):
        ...
        if (self.odomCheck):
            ...
            if (self.angularDistance < 0):
                Y2 = Y2 - 2 * math.pi if (Y2 - Y1 > 0.1) else Y2
            else:
                Y2 = 2 * math.pi + Y2 if (Y1 - Y2 > 0.1) else Y2
            #


            angularTraveled = math.fabs(Y2 - Y1)
            ...
            if (angularTraveled > self.prevAngularTraveled + 0.05 or angularTraveled >= math.fabs(self.angularDistance)):
                rospy.logdebug(
                ...
                self.prevAngularTraveled = angularTraveled

            if angularTraveled >= math.fabs(self.angularDistance) and self.angularDistanceAlert is False:
                rospy.loginfo("Walker: setting angular distance alert True")
                self.angularDistanceAlert = True

    def moveAngular(self, distance, ignoreCollisionAlert=False, angularSpeed = 0.2):
        '''global self.odomCheck
        global angularDistance
        global self.angularDistanceAlert
        global self.collisionAlert
        ...
        walkPub = rospy.Publisher('/komodo_1/cmd_vel', Twist, queue_size=1)
        ...
        while not (self.angularDistanceAlert):
            if (self.collisionAlert and not ignoreCollisionAlert):
                ...
                break
            walkTwist.angular.z = angularSpeed * math.pi
    ...
    def walkUntilCollision(self, cmd):
        ...
        self.moveAngular((cmd.angle * math.pi) / 180, True)
        rospy.sleep(3)
        ...
        self.walkPub.publish(res)
        rospy.logdebug("published result {}".format(res.result))
        # ------------------


if __name__ == '__main__':
    try:
        rospy.init_node('Walk', anonymous=True, log_level=rospy.DEBUG)
        walker = Walker()
        rospy.spin()
