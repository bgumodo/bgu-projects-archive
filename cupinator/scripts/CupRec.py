...
class ShapeContextCupRec:
    ...
    upper_red = np.array([8, 255, 170])
    lower_high_red = np.array([172, 150, 50])
    upper_high_red = np.array([179, 255, 170])
    ...
    def compute_shapecontext(self, cv_img):
        ...
        while ((len(input_samples) == 0) and (sample_th >= 40)):
            ...
            if (len(input_samples) > 5):
                sample_th *= 2
                rospy.logdebug("{}: Too many contours. Re-sampling image (sample count threashold increased {})".format(fname, sample_th))
        sc = cupRec.SC.SC()
    ...
    def cupRec_single_query(self, cv_img):
        ...
        contour_match_list = self.compute_contours_match_cost(cv_img, -1)
        min_cost = sys.maxint
        min_cost_idx = -1
    ...
    def compute_contours_match_cost(self, cv_img, image_key):
        """
        ...
        rospy.logdebug("{}: Computing shape context for image (key = {})".format(fname, image_key))
        ...
        for (contour_idx, (contour, contour_SC)) in enumerate(contours_SC):
            ...
            rospy.loginfo("{}: Finding best match for contour_{} (image key = {}). Stand by...".format(fname, contour_idx, image_key))
            ...
            for (index, (DB_contour, DB_contour_SC)) in enumerate(self.shapecontext_DB):
                ...
                rospy.logdebug("{}: Match cost to DB-contour #{} = {}".format(fname, index, contour_cost))
    ...
    def aggregate_image(self, image_data):
        ...
        contour_list = self.compute_contours_match_cost(cv_img, image_data[1])
